...
import { dispatch as d3dispatch } from 'd3';

const cloudRadians = Math.PI / 180, cw = 1 << 11 >> 5, ch = 1 << 11;

function TagCloudDrawingService() {
    var size = [256, 256],
      dispatcher = d3dispatch('word', 'end'),
      random = Math.random,
      cloud:any = {};
    ...
    cloud.draw = function(words) {
        if (!words) { return; }
        var cloudTagRotations = [0, 20, 30, 40];

        function getRotation() {
            var num = ~~(Math.random() * 4 + 1);
            var rotation = cloudTagRotations[num - 1] || 0;
        ...
        var contextAndRatio = getContext(getNewCanvas()),
        ...
        function step() {
            ...
            if (i >= n) {
              dispatcher.apply('end', this, [tags, bounds]);
            }
        }
    };

    cloud.setSize = function(width, height) {
        size[0] = width;
        size[1] = height;
    };
...
export default TagCloudDrawingService
